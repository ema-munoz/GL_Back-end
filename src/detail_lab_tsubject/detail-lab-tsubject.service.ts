...
export class DetailLabTsubjectService {
	constructor(
        @InjectRepository(DetailLabTsubjectEntity) private dtltSubjectsRepo: Repository<DetailLabTsubjectEntity>,
    ) { }
	...
	async findOne(id: number) {
		const dtltSubjects = await this.dtltSubjectsRepo.findOne(
			{
				where: {id:id}
			}
		)
		if (dtltSubjects == null) {
			throw new NotFoundException("Detalle Laboratorio no encontrado");
		}
		return dtltSubjects;
	}
	...
	create(payload: CreateDetailLabTsubjectDto) {
		const newDtltSubjects = this.dtltSubjectsRepo.create(payload);
	...
	async update(id: number, payload:  UpdateDetailLabTsubjectDto) {
        const dtltSubjects = await this.dtltSubjectsRepo.findOne({ where: {id:id}});
        if (dtltSubjects === null) {
            throw new NotFoundException('Detalle Laboratorio no encontrado');
        }
